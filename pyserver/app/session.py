...
async def run(session_id: str, prompt: str, mode: str = "") -> AsyncGenerator[str, None]:
    ...
    # Reference and script skills are files in the workspace, and the agent is
    # told to read them with read_file - so they have to exist before the turn
    # starts, not only after run_python has been called once.
    ws_dir = workspace.root(crow["char_key"])
    pyexec.install_skills(ws_dir)
