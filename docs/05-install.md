...
$got  = (Get-FileHash Risu.Hina.0.5.2.Auto.Install.Package.zip -Algorithm SHA256).Hash.ToLower()
...
```powershell
Expand-Archive Risu.Hina.0.5.2.Auto.Install.Package.zip -DestinationPath D:\code -Force
```
...
health     {"service": "risu-hina", "version": "0.5.2", "ok": true, "agentReady": false, ...}
```
...
```powershell
Expand-Archive Risu.Hina.0.5.2.Auto.Install.Package.zip -DestinationPath E:\apps -Force
...
```bash
unzip Risu.Hina.0.5.2.Auto.Install.Package.zip -d /opt
