...
$got  = (Get-FileHash Risu.Elf.0.2.0.Auto.Install.Package.zip -Algorithm SHA256).Hash.ToLower()
...
```powershell
Expand-Archive Risu.Elf.0.2.0.Auto.Install.Package.zip -DestinationPath D:\code -Force
```
...
health     {"service": "risu-elf", "version": "0.2.0", "ok": true, "agentReady": false, ...}
```
...
```powershell
Expand-Archive Risu.Elf.0.2.0.Auto.Install.Package.zip -DestinationPath E:\apps -Force
...
```bash
unzip Risu.Elf.0.2.0.Auto.Install.Package.zip -d /opt
