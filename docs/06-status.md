...
| GitHub 릴리스 | **v0.8.1 · BETA Latest** (2026-08-27 23:19) · v0.8.0 · v0.7.2 · v0.7.1 · v0.7.0 (2026-08-27 20:32, 자산 4개, 내가 `gh release create` 로 직접 — auto 모드에서도 통과했다; 노트는 `tools/bundle.py` 가 `release/` 를 비우므로 scratchpad 에 쓴다) · v0.6.2 · v0.6.1 · v0.6.0 · v0.5.2 · … · v0.1.0 | `gh release create` 는 auto 모드 분류기가 막는다 — 수동 권한 모드에서는 내가 직접 실행(0.3.1·0.3.2). zikmunt-pc 는 0.3.2 배포·검증됨, raw 주소도 0.3.2 |
