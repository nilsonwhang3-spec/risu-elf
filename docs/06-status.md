# 06. 구현 상태 — 2026-08-29 기준 (v0.9.4 BETA, Risu Hina)
...
**코드 상태**: master = **v0.9.4 BETA**(§1-13 재오픈 3-way 병합 · §1-12 중간 캐시가 연결을 막던 것(POST 프로브·no-store) · §1-11 웹 검색 툴 카드 3택1 · §1-10 내장 검색 실측·모바일·플러그인 재로드 진단 · §1-9 검색 · §1-8 라운드 10 · §1-7 · §1-6 · §1-5; docs/07 플래닝은 여전히 대기) — 게이트 ALL GREEN. 0.7.0 은 minor 가 바뀌어 **버전 게이트가 걸린다**: 백엔드를 올리면 RisuAI 쪽 플러그인도 `+` 로 올려야 한다(헤더가 안내).
...
- **0.9.3 (큰 파일 업로드)**: 사용자가 138MB `.charx` 를 워크스페이스에 올리려다 실패. 원인은 크기 한계 두 겹 — 플러그인이 60MB 초과를 거부(`tab-files.ts` SOLO), 백엔드 본문 상한 64MB(`MAX_BODY_BYTES`). 캐릭터 charx 는 140~180MB 가 정상이라 한 요청으로는 절대 못 간다(앞단 릴레이도 못 받는다). **분할 업로드** 신설: `POST /files/upload-chunk`(upload-many 와 같은 바이너리 프레이밍, 헤더 `{charKey,dir,name,rel,offset,total,last,extract}`), 서버는 `<name>.part` 에 이어붙이고 **오프셋을 디스크 실제 크기와 대조**(재시도·경합이 섞여 그럴듯한 손상 파일을 만드는 것을 막는다), 마지막 조각에서 크기 검증 후 rename. `.part` 는 목록에서 제외. 플러그인은 배치(16MB)보다 큰 파일을 `file.slice()` 로 잘라 보낸다 — 180MB 를 한 배열로 읽지 않는다. **실측: 실제 138MB charx 9조각 1.0초, 크기 일치.** 회귀 테스트 6건(조각 수신·미완성은 목록에 없음·오프셋 어긋남 거부·완성·바이트 일치·크기 불일치 거부).
